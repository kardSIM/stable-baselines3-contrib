...
class FQF(OffPolicyAlgorithm):
    ...
    def __init__(
        self,
        policy: Union[str, type[FQFPolicy]],
    ...
    def _create_aliases(self) -> None:
        ...
        self.entropy_coef = self.policy.entropy_coef        
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                next_greedy_actions = next_q_values.argmax(dim=1, keepdim=True)
                # Follow greedy policy: use the one with the highest Q values

                next_greedy_actions = next_greedy_actions[:, None, :].expand(batch_size, self.n_quantiles, 1)
                next_quantiles = next_quantiles.gather(dim=2, index=next_greedy_actions).squeeze(dim=2)                # Make "n_quantiles" copies of actions, and reshape to (batch_size, n_quantiles, 1)
                # 1-step TD target
